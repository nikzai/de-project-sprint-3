...
def upload_data_to_staging(filename, date, pg_table, pg_schema, ti):
    ...
    if 'payment_amount' in df.columns:
        df.loc[df['status'] == 'refunded', 'payment_amount'] = -df['payment_amount']

    postgres_hook = PostgresHook(postgres_conn_id)
    engine = postgres_hook.get_sqlalchemy_engine()
    row_count = df.to_sql(pg_table, engine, schema=pg_schema, if_exists='append', index=False, method=insert_on_conflict_update)
...
with DAG(
    ...
    update_f_customer_retention = PostgresOperator(
        ...
        postgres_conn_id=postgres_conn_id,
        sql="CALL mart.proc_customer_retention_insert('week')"
    )
